...
function buildInvoiceDateFields(dateValue) {
  if (!dateValue) {
    return { display: "", localDateTimeString: null };
  }
...
export default class ArAging extends NavigationMixin(LightningElement) {
  @track asOfDate = getTodayInUserTimeZone();
  ...
  wiredTeams(result) {
    ...
    if (result.data) {
      ...
      this.error =
        result.error.body?.message || "Error loading available teams";
    }
  }
  ...
  get pageInfo() {
    ...
    const endRecord = Math.min(
      this.currentPage * RECORDS_PER_PAGE,
      this.totalRecords
    );
  ...
  async showInvoiceDetails(agingBucket, title) {
    ...
    try {
      ...
      this.modalData = result.map((invoice) => {
        const { display, localDateTimeString } = buildInvoiceDateFields(
          invoice.invoiceDate
        );

        return {
          ...invoice,
          invoiceDateDisplay: display,
          invoiceDateLocal: localDateTimeString,
  ...
  handleDownloadCSV() {
    ...
    try {
      ...
      const fileName = `${this.modalTitle.replace(/\s+/g, "_")}_${ISO_DATE_FORMATTER.format(
        new Date()
      )}.csv`;
